...
if __name__ == "__main__":
    try:
        ...
        for run_idx in range(NUM_RUNS):
            ...
            # 每次设不同种子
            torch.manual_seed(2024 + run_idx)
            np.random.seed(2024 + run_idx)

            seld_score = main(sys.argv)
            all_scores.append(seld_score)

        all_scores = np.array(all_scores)
        mean_score = np.mean(all_scores, axis=0)
        std_score = np.std(all_scores, axis=0)

        print("\n========= Final Statistical Results (3 runs) =========")
        print(f"Mean SELD score: {mean_score:.4f}")
        print(f"Std SELD score : {std_score:.4f}")
        print("======================================================")
