...
pred = os.path.join(params['dcase_output_dir'], '8_1_dev_split0_accdoa_foa_20250408151818_val/fold6_room2_mix041.csv')
...
ref_dir = os.path.join(params['dataset_dir'], 'metadata_dev/dev-val')
aud_dir = os.path.join(params['dataset_dir'], 'foa_dev/dev-val')
...
ref_filename = os.path.basename(pred)#fold5_room2_mix041.csv
...
nb_feat_frames = int(len(audio) / float(feat_cls._hop_len))
stft = np.abs(np.squeeze(feat_cls._spectrogram(audio[:, :1],nb_feat_frames)))
