...
from conformer.encoder import ConformerBlocks
...
class CNNAFF_COM(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.params = params
        ...
        self.branch1 = BasicConv2d(in_feat_shape[1], params['nb_cnn2d_filt'], kernel_size=1)  # (7,64)
        self.maxpool1 = nn.AvgPool2d(kernel_size=(5, 4))

        self.branch2 = nn.Sequential(
            BasicConv2d(in_feat_shape[1], params['nb_cnn2d_filt'], kernel_size=1),
            BasicConv2d(params['nb_cnn2d_filt'], params['nb_cnn2d_filt'], kernel_size=3, padding=1)   # (7,64)
        )
        self.maxpool1 = nn.AvgPool2d(kernel_size=(5, 4))

        self.fuse_mode = AFF(channels=params['nb_cnn2d_filt'])
        self.maxpool2 = nn.AvgPool2d(kernel_size=(1, 4))
        self.maxpool3 = nn.AvgPool2d(kernel_size=(1, 2))


        self.conformer = ConformerBlocks(encoder_dim=128, num_layers=2)
        self.fnn_list = torch.nn.ModuleList()
        if params['nb_fnn_layers']:
            for fc_cnt in range(params['nb_fnn_layers']):
        ...
        self.fnn_list.append(
            nn.Linear(params['fnn_size'] if params['nb_fnn_layers'] else self.params['rnn_size'], out_shape[-1],
                      bias=True))
    ...
    def forward(self, x):
        ...
        out1 = self.maxpool1(branch1)
        branch2 = self.branch2(x)
        out2 = self.maxpool1(branch2)

        x=self.fuse_mode(out1,out2)
        x = self.maxpool2(x)
        x = self.maxpool3(x)
...
class ResBlock(nn.Module):
    def __init__(self, in_channels,out_channels,downsample=None):
        ...
        # 3×3卷积
        self.branch1 = BasicConv2d(in_channels, out_channels, kernel_size=3)
        # 3×3卷积
        self.branch2 = BasicConv2d(out_channels, out_channels, kernel_size=3)
    ...
    def forward(self, x):
        ...
        # out += identity
        #out = self.fuse_mode(identity, out)
        #out = self.relu(out)
        out = F.relu(self.fuse_mode(identity, out))

        return out

class ResFusion_COM(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.params = params
        ...
        self.stem = BasicConv2d(in_feat_shape[1], 16, kernel_size=3)

        self.branch1 = ResBlock(16, 32)
        self.maxpool1 = nn.AvgPool2d(kernel_size=(5, 4))

        self.branch2 = ResBlock(32, 64)
        self.maxpool2 = nn.AvgPool2d(kernel_size=(1, 4))

        self.branch3 = ResBlock(64, 128)
        self.maxpool3 = nn.AvgPool2d(kernel_size=(1, 4))
...
class InceptionBlock(nn.Module):
    def __init__(self, in_channels,out_channels,msoutchannels,downsample=None):
        super(InceptionBlock, self).__init__()

        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1,stride=1, padding=0)
        self.branch1 = BasicConv2d(in_channels, msoutchannels, kernel_size=1)  # (7,64)
        self.branch2 = nn.Sequential(
            BasicConv2d(in_channels, msoutchannels, kernel_size=1),
            BasicConv2d(msoutchannels, msoutchannels, kernel_size=3, padding=1)  # (7,64)
        )
        self.downsample = downsample
        self.fuse_mode = AFF(channels=out_channels)
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            identity = self.downsample(x)
        finalout = F.relu(self.fuse_mode(identity, out))
        return finalout
class InceptionFusion_COM(torch.nn.Module):
    def __init__(self, in_feat_shape, out_shape, params):
        super().__init__()
        self.nb_classes = params['unique_classes']
        self.params = params
        ...
        self.stem = BasicConv2d(in_feat_shape[1], 16, kernel_size=3,padding=1)

        self.branch1 = InceptionBlock(16, 32, 16)
        self.maxpool1 = nn.MaxPool2d(kernel_size=(5, 4))

        self.branch2 = InceptionBlock(32, 64 , 32)
        self.maxpool2 = nn.MaxPool2d(kernel_size=(1, 4))

        self.branch3 = InceptionBlock(64, 128, 64)
        self.maxpool3 = nn.MaxPool2d(kernel_size=(1, 2))

        self.conformer = ConformerBlocks(encoder_dim=256, num_layers=2)
        self.fnn_list = torch.nn.ModuleList()
        if params['nb_fnn_layers']:
            for fc_cnt in range(params['nb_fnn_layers']):
                self.fnn_list.append(
